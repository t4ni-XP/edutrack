  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const studentIdsToDelete = existingEnrollments
        .map((e) => e.studentId)
        .filter((studentId) => !newStudentIds.has(studentId));
      ...
      const newTutorIds = new Set(data.tutorAssignments.map((t) => t.tutorId));

      const tutorIdsToDelete = existingTeachings
        .map((t) => t.tutorId)
        .filter((tutorId) => !newTutorIds.has(tutorId));
    ...
    return NextResponse.json(serializeClass(result));
  } catch (error) {
